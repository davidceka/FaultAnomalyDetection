...
new_training_set = training_set(training_set.FaultLabel == 0, 1:n_features); % only no faulty, then we will append original + synthethic data for faulty test


for j=1:8 % class 0 is the majority class
    X = training_set(training_set.FaultLabel == j, 2:n_features); % get only rows of training set with engine failure (1)
    X = table2array(X);
